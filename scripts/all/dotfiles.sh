...
# Check if directory needs updating (compares checksums of all files)
directory_needs_update() {
    local src_dir="$1"
    local dest_dir="$2"
    ...
    # Get sorted list of files and their hashes from both directories
    local src_hash=$(find "$src_dir" -type f -exec shasum {} \; 2>/dev/null | sort | shasum | awk '{print $1}')
    local dest_hash=$(find "$dest_dir" -type f -exec shasum {} \; 2>/dev/null | sort | shasum | awk '{print $1}')
...
install() {
    ...
    if [[ -d "${DOTFILES_SOURCE}/home_bin" ]]; then
        # Check if bin directory needs updating
        if directory_needs_update "${DOTFILES_SOURCE}/home_bin" "${HOME}/bin"; then
            ...
            # Count files that would have been processed for skip count
            local bin_items=($(find "${DOTFILES_SOURCE}/home_bin" -maxdepth 1 -type f -o -type d ! -path "${DOTFILES_SOURCE}/home_bin"))
            files_skipped=$((files_skipped + ${#bin_items[@]}))
        fi
    fi

    # Note: .gitconfig is generated by the gitconfig module from template
